...
192-bit outward continuation re-certifies all 63 WDBC and digits brackets.
  ...
  forecasts abstain before randomized probing.  Frozen computational
  refinements further reduce Green-operator work and recover certificates at
  shorter reference paths without changing the issued brackets.  The resulting
  verifier is selective and explicit about its numerical and probabilistic
  obligations.
\end{abstract}
...
\begin{enumerate}
  ...
  dual direction.
  For scaled momentum, a chronological forcing-subspace corollary closes each
  parameter checkpoint through row operators
  $\Pi_\theta K_i\mathcal B$.  It removes optimizer-state directions that
  neither generate the nonlinear remainder nor affect the neural event, and it
  replaces a global quadratic discriminant by an explicit causal recursion.
...
$8.75\times$ Green reduction, transpose-free causal-row audit, and eight
deterministic output releases reuse frozen analytic envelopes.  The causal-row
theorem reduces matched post-reference sweeps $144\to98$, but its current
process-isolated implementation is 1.17$\times$ slower in wall time than the
released panel because deterministic jets and repeated setup dominate.  The
result isolates the next scaling target; it does not settle neural-jet
construction at modern scale.
