...
applications; staged fallback retains 15/15.  On an issued $H=26$ Transformer
case, composing prefix streaming, four forward-only Green probes, and analytic
jets reproduces the sealed bracket in 9.21\,s median over three separately
launched runs, with no Green transpose or randomized output operator.  This is
$378\times$ below the historical fixed-$q=8$ cross-batch median; matched direct
continuation remains $30.9\times$ faster over 26 updates and $2.48\times$ faster
over 300.  The resulting verifier is
...
\begin{enumerate}
  ...
  Their complete composition reproduces a sealed $H=26$ bracket in 9.21\,s
  median using four forward Green probes, no transpose, and no randomized
  output operator.
\end{enumerate}
...
eight of those cases.  Their complete composition turns one sealed Transformer
