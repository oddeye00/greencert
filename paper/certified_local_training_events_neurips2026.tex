...
The theorem encloses the realized nonlinear trajectory over a finite window
and transports that enclosure to a persistent event on a fixed evaluation set.
Across four frozen or outcome-sealed studies, all 83 issued brackets contain their subsequently revealed crossings;
independent outward continuation retains all 63 WDBC and digits brackets
under the stated arithmetic assumptions.  In matched
comparisons, unsigned propagation loses the 147-step digits bracket and eight
of the nine fixed-radius Transformer certificates.  Both inaccurate finite digits
forecasts abstain before randomized probing.  The result is a selective
verifier that distinguishes an accurate local clock from one whose future
trajectory has actually been certified.
\end{abstract}
...
\begin{enumerate}
  ...
  implementation error.  The theorem preserves the known signed response and
  certifies only the nonlinear remainder; time-resolved, relinearized,
  transported-envelope, and scaled-momentum corollaries sharpen
  this closure without changing the event being certified.
  \item We give a matrix-free, anytime implementation based on HVP/VJP
  products.  Shared Gaussian blocks, predictable probability spending, and
  verified residual identities support adaptive stopping while retaining an
  explicit family-wise guarantee.
  ...
  63 non-modular brackets survives independent outward continuation, and
  both inaccurate finite digits clocks are rejected before randomized probing.
  \item We measure how defect direction affects certificate issuance.  A matched unsigned
  construction retains only one of nine Transformer certificates and loses the
  sealed 147-step digits event.  Computational refinements reduce Green and
  output-operator cost without changing the issued brackets; for example, a
  frozen 15-case audit reduces Green applications from 560 to 64
  ($8.75\times$) while retaining all 15 brackets.
\end{enumerate}

At a high level, the verifier has four stages: build a checkpoint-local clock,
propagate its realized signed defect, certify the remaining nonlinear
trajectory error, and transport the resulting tube through neural-output
margins to a persistent event bracket.  The next two sections develop these
stages in that order; the implementation refinements are introduced only when
they affect the guarantee or its cost.
...
If storing a coordinate change rounds the initial state, the physical
checkpoint is retained by an explicit first-step injection rather than
silently replacing the anchor; Corollary~\ref{cor:physical-anchor} gives the
corresponding response-error interface.
...
\paragraph{Scaled-momentum forcing subspace: chronological closure.}
...
Writing $\Pi_\theta$ for parameter projection, the global corrected closure may
therefore query $\Pi_\theta\bar K_H\mathcal B$ instead of the full
$\bar K_H$ (Appendix Corollary~\ref{cor:parameter-forcing}).  More sharply,
let $\bar K_i$ be the causal row ending at checkpoint $i+1$ and
$\kappa_i^\theta\ge\|\Pi_\theta\bar K_i\mathcal B_i\|$.  If $Y_{i+1}$ bounds
the known signed response and unresolved forcing through time $i$, the
parameter radii satisfy the explicit recursion
\begin{equation}
 r_0=0,\qquad
 r_{i+1}=Y_{i+1}+\kappa_i^\theta
 \left\{\sum_{k=1}^{i}
 \left(\frac{L_kr_k^2}{2}\right)^2\right\}^{1/2}.
 \label{eq:causal-row-main}
\end{equation}
Appendix Corollary~\ref{cor:causal-parameter-row} proves chronological closure.
It has no global discriminant: later gain or curvature cannot invalidate an
already enclosed earlier checkpoint.  Proposition~\ref{prop:row-probe} obtains
all row norms from the checkpoint outputs of one direct Gaussian pass, without
a transpose product.
...
\begin{proposition}[Simultaneous causal-row direct-image enclosure]
\label{prop:row-probe}
Let $T_0,\ldots,T_{H-1}$ be fixed row operators on a common Euclidean input
space.  Draw independent standard Gaussian vectors $g_1,\ldots,g_m$ and choose
$\delta_i>0$ with $\sum_i\delta_i\le\delta$.  Define
\begin{equation}
 c_i=\Phi^{-1}\!\left(\frac{1+\delta_i^{1/m}}2\right),\qquad
 \widehat\kappa_i=\frac{\max_{\ell\le m}\|T_i g_\ell\|}{c_i}.
 \label{eq:causal-row-probe}
\end{equation}
Then, in exact arithmetic, with probability at least $1-\delta$,
$\|T_i\|\le\widehat\kappa_i$ simultaneously for every $i$.
For causal Green rows, all $T_i g_\ell$ are checkpoint outputs of the same
forward sweep.
\end{proposition}

\begin{proof}
Fix a top right singular vector $v_i$ of $T_i$.  Since
$\|T_i g_\ell\|\ge\|T_i\||v_i^\top g_\ell|$, failure for row $i$ requires all
$m$ absolute standard-normal projections to be below $c_i$, an event of
probability $\delta_i$.  A union bound over rows proves the claim.
\end{proof}
...
candidate, or certificate.  In a \emph{prospective}, \emph{outcome-sealed}
study, the certificate record is sealed before the future trajectory is
opened; the \emph{outcome join} then compares that record with the revealed
crossing.  An \emph{issued} candidate satisfies every closure and event-margin
condition; otherwise \method{} abstains.  A \emph{construction abstention}
occurs when construction cannot be completed under the frozen protocol.
Post-seal comparisons and development audits of methodological refinements
are reported separately and never alter prospective issuance.
...
The 63 WDBC and digits brackets additionally receive independent outward
continuation audits under the arithmetic assumptions in
Appendix~\ref{app:repaired-arithmetic}.  The 20 Transformer certificates remain high-confidence
...
The result survives an independent outward replay of 40 unique
...
radius is $4.47\times10^{-13}$, minimum strict output slack is
...
the retained brackets describe exact-real continuation from the stored dyadic
checkpoint under the stated arithmetic assumptions.  The preceding
PyTorch/BLAS training program and population accuracy are not certified.

Direct outward continuation provides an independent validation baseline on
the small tanh networks, but it
...
direct outward runs therefore audit the issued small-model brackets rather
...
A Green-independent outward replay then encloses seven exact-real
...
is $1.46\times10^{-5}$.  Under the same arithmetic assumptions, this validates
continuation from stored dyadic checkpoints, not the preceding training
execution or the randomized Green inequality.
...
The same directional jet supplies a second reuse opportunity.
Proposition~\ref{prop:transported-neural-envelope} transports stage values and
parameter geometry from the three-sweep center along its known signed
correction, then expands only the unknown shadowing radius.  In a separately
frozen equivalence audit, the transported envelope dominates a fresh
corrected-center envelope at all 9,420 checked stage, geometry, and derivative
entries and preserves all four brackets above, including all three
nondevelopment cases.  The largest stage and derivative dominance ratios are
$1+7.32\times10^{-13}$ and $1+5.53\times10^{-14}$, respectively.  This removes
a duplicate full stage/geometry audit from the mathematical construction; we
make no wall-time claim from the non-isolated cohort run.
...
Scaled-momentum structure gives a second, orthogonal reduction.  The initial
global forcing-subspace audit restricts nonlinear gain to
$\Pi_\theta K_H\mathcal B$, preserves all 15 brackets, and reduces the staged
direct/Gram total from 112 to 96 sweeps.  The chronological row theorem then
removes the remaining global-in-time compression.  Its protocol was frozen
after one disclosed development row and before structured-row probes were
drawn for the other 14 rows.  All 15 cases issue, including 14/14 holdouts; 13
stop after four probes and two after eight, and every bracket equals its sealed
comparator.  Relative to the released Gram panel, random norm-estimation work
falls from 128 forward/transpose sweeps to 68 forward sweeps
($1.88\times$).  Including deterministic closure responses gives
$129\to83$ ($1.55\times$), and matched full post-reference accounting gives
$144\to98$ ($1.47\times$).  All 64 transpose sweeps disappear.  An independent
scalar implementation recomputes every calibration, radius, margin, bracket,
cache identity, and source hash; the minimum strict event slack is
$1.45\times10^{-6}$ and the largest radius uses only
$1.28\times10^{-6}$ of its derivative domain.

This operator reduction has not yet become an end-to-end speedup.  The
process-isolated row audit takes 1085.63 wall seconds versus 927.82 seconds for
the released panel because it evaluates sharper local jets and repeats common
setup for the two eight-probe extensions.  The result moves the measured
bottleneck away from Green transposes and into deterministic jet construction
and orchestration; it does not relabel lower operator count as lower wall time.
...
\begin{figure}[t]
  ...
  (c) Matched post-reference linearized-sweep accounting for the released Gram
  panel and the frozen causal-row audit.}
...
Componentwise shadowing rotates local errors into finite-time growth directions,
and inductive containment builds step-local boxes
\citep{vanvleck2000componentwise,hayes2003containment}.  These constructions
sharpen orbit enclosures but do not fix the initial state to a realized neural
optimizer checkpoint and transport the resulting tube through neural margins.
...
\citep{becker2001optimal}.  Adjoint analysis has also produced computable error
estimates for the first time a differential-equation functional reaches a
threshold \citep{chaudhry2021threshold}; goal-oriented surrogate learning now
controls path-space observables including mean first-hitting times
\citep{zou2026goaloriented}.  \method{} instead encloses the
realized discrete trajectory and returns a strict persistent-event bracket or
abstention, without assuming that the local event-time linearization is
accurate or replacing the optimizer by a learned dynamical surrogate.  Our forcing-subspace result uses the exact
...
\midrule
Chow--Lin--Palmer; Chow--Palmer;\newline Van Vleck; Hayes--Jackson
\citep{chow1989shadowing,chow1992numerical,vanvleck2000componentwise,hayes2003containment}
& numerical pseudo-orbit & right-inverse/Newton correction
& shadowing orbit and componentwise/containment bound \\
...
First-threshold error analysis \citep{chaudhry2021threshold}
& differential-equation trajectory & adjoint residual and event-time linearization
& a posteriori first-threshold timing error estimate \\
...
The present complete certificates cover shallow tanh MLPs and a one-block,
13,792-parameter Transformer.  A 102,400-parameter LayerNorm/AdamW audit
already transports the required derivatives; extending complete closure to
deep networks and stochastic optimization is the main technical next step.
At 1.01M parameters, the matrix-free core takes 11.37 hours before neural-jet
construction.  At the fully composed scale, however, the 13,792-parameter
$H=26$ verifier runs in 9.21\,s median, turning an hours-scale historical
construction into a seconds-scale proof object.  Matched direct continuation
is still $30.9\times$ faster over that short window.  The events are defined on
fixed evaluation sets and coverage is conditional on issuance, so these are
finite-set event guarantees rather than population guarantees.

The numerical guarantees have two levels.  All 63 WDBC and digits certificates
survive direct outward continuation from stored checkpoints using 192-bit Arb
endpoints and explicit binary64 error bounds.  The latter assume ordinary
round-to-nearest dot products with gradual underflow, rather than certify an
arbitrary BLAS implementation (Appendix~\ref{app:repaired-arithmetic}).  The
204 amplified-secant jets are outward at 192 bits conditional on stored dyadic
inputs.  The upstream Transformer Green products, derivative envelopes, and
output margins remain float64/ideal-PRNG calculations.  The directional-block
remainder is specialized to the normalization-free Transformer and uses
analytic block majorants rather than outward-rounded coefficient arithmetic;
its frozen post-release audit is evidence about the method, not an additional
prospective coverage cohort.  Finally, the causal-row theorem reduces matched
post-reference sweeps $144\to98$, while its current process-isolated
implementation is 1.17$\times$ slower because deterministic jets and repeated
setup dominate.  These measurements locate the remaining systems bottleneck
in neural-jet construction rather than dense Green storage.
...
mixed jet reproduces every local bound.  Chronological forcing-subspace closure
then retains all 15 brackets across 14 holdouts, removes all 64 transpose
sweeps, and reduces matched post-reference work $144\to98$.  Their complete
composition turns one sealed Transformer certificate from an hours-scale
historical construction into a reproducible 9.21-second proof object without
changing its bracket.
...
\begin{corollary}[Physical checkpoint under an inexact stored anchor]
\label{cor:physical-anchor}
Let $x_a$ be the physical checkpoint and let the stored reference start at
$c_0$, not necessarily equal to $x_a$.  Define
\begin{equation}
 a_0=G(x_a)-G(c_0),\qquad
 \bar s=s+\mathcal I_0a_0,
 \label{eq:physical-anchor-injection}
\end{equation}
where $\mathcal I_0a_0=(a_0,0,\ldots,0)\in U$.  On times $1,\ldots,H$, let
$h_j=x_{a+j}-c_j$, and use the virtual value $h_0=0$ only in the sequence
recurrence.  Put $N_+(h)_0=0$ and $N_+(h)_j=N_j(h_j)$ for $1\le j<H$.
Then
\begin{equation}
 h=K_H\bar s+K_HN_+(h).
 \label{eq:physical-anchor-fixedpoint}
\end{equation}
Theorem~\ref{thm:green} applies on times $1,\ldots,H$ with $s$ replaced by
$\bar s$ and $q_0=0$.  In particular, if
$\|s-\widetilde s\|_U\le\sigma$, $\|a_0-\widehat a_0\|\le\gamma$, and
the computed response $\widetilde z_0=0$ satisfies
\begin{equation}
 d_j=\widetilde z_{j+1}-J_j\widetilde z_j-\widetilde s_j
       -\mathbf 1_{j=0}\widehat a_0,\qquad \|d\|_U\le\tau,
\end{equation}
then $\alpha=\kappa(\sigma+\gamma+\tau)$ is valid.  Any event test at time
zero must use an enclosure at the physical $x_a$, not the virtual anchor.
\end{corollary}

\begin{proof}
The first physical error is $h_1=G(x_a)-c_1=s_0+a_0$.  For $j\ge1$,
$h_{j+1}=J_jh_j+s_j+N_j(h_j)$.  These recurrences give
Eq.~\eqref{eq:physical-anchor-fixedpoint}; $J_0$ is immaterial because the
virtual initial error is zero.  Substitution of the computed recurrence
gives $\widetilde z=K_H(\widetilde s+\mathcal I_0\widehat a_0+d)$ and hence
the stated $\alpha$ bound.  The proof of Theorem~\ref{thm:green} now applies
unchanged: only inputs $1\le j<H$ require the derivative envelope.  The
virtual convention makes no claim about $x_a-c_0$ or its output at time zero.
\end{proof}

For example, consider scaled momentum
$G(\theta,w)=(\theta-\mu w-\eta\nabla F(\theta),\mu w+\eta\nabla F(\theta))$.
If the original stored checkpoint is $(\theta_0,v_0)$ and its scaled
reference stores $\bar w_0=\operatorname{fl}(\eta v_0)$, then the physical
scaled checkpoint is $(\theta_0,\eta v_0)$, with the product interpreted in
exact arithmetic.  Writing $\delta=\eta v_0-\bar w_0$ gives the exact source
$a_0=(-\mu\delta,\mu\delta)$.  This identity needs no Hessian product:
momentum enters the map affinely.  The known source may be injected in the
first signed sweep, or its homogeneous response added to an existing sweep;
rounding in either construction belongs in $\tau$, and any unrepresented
source tail belongs in $\gamma$.  The operator $K_H$ is unchanged, so this
deterministic correction uses the same operator-norm event.  Without signed
propagation one may instead set $\widehat a_0=0$ and charge
$\gamma\ge\|a_0\|$.  For a general nonlinear map, replacing $a_0$ by
$J_0(x_a-c_0)$ requires an enclosure of the omitted nonlinear residual.
...
\begin{proposition}[Directionally transported neural envelope]
\label{prop:transported-neural-envelope}
Let the corrected reference at checkpoint $j$ be
$b_j=c_j+v_j$, write $a_j=\Pi_\theta v_j$, and suppose the unknown parameter
error is at most $E\le\rho$.  For every intermediate neural stage $h_\ell$,
assume a directional jet on $\theta_j+ta_j$, $0\le t\le1$, gives
\begin{equation}
 A_{\ell,j}\ge
 \sup_{0\le t\le1}\left\|\frac{d}{dt}
 h_\ell(\theta_j+ta_j)\right\|.
 \label{eq:stage-transport}
\end{equation}
For each disjoint parameter block, let $W_{r,j}$ and $V_{r,j}$ be its values
in $c_j$ and $v_j$.  Define transported inputs
\begin{equation}
 \widehat H_{\ell,j}=\|h_\ell(\theta_j)\|+A_{\ell,j},
 \qquad
 \widehat G_{r,j}=\|W_{r,j}\|_2+\|V_{r,j}\|_F,
 \label{eq:transported-envelope-inputs}
\end{equation}
with Euclidean norms for vector blocks.  If the ordinary ball-valid neural
envelope is coordinatewise nondecreasing in its stage inputs, parameter
geometry, and ball radius, then evaluating it at
$(\widehat H_j,\widehat G_j,\rho)$ bounds the same derivatives throughout
$B(\Pi_\theta b_j,\rho)$ as a fresh corrected-center construction.

In particular, if it returns bounds $(L_{1,j},L_{2,j},L_{3,j})$, then the
corrected-path closure may use
\begin{equation}
 M_j=\sqrt2\eta\left(2L_{1,j}^3+	frac32L_{1,j}L_{2,j}
 +\sqrt2L_{3,j}\right),
 \label{eq:transported-envelope-drift}
\end{equation}
and a true-versus-competitor logit margin pays uncertainty
$\sqrt2L_{1,j}E$.  The known correction $a_j$ is absorbed into the center and
is not charged again in the event radius.
\end{proposition}

\begin{proof}
The fundamental theorem of calculus and Eq.~\eqref{eq:stage-transport} give
$\|h_\ell(\theta_j+a_j)\|\le\widehat H_{\ell,j}$.  For every matrix block,
$\|W_{r,j}+V_{r,j}\|_2\le\|W_{r,j}\|_2+\|V_{r,j}\|_2
\le\widehat G_{r,j}$; the vector-block argument is identical.  These inputs
therefore dominate the exact stage values and geometry at the corrected
center.  Coordinatewise monotonicity of each nonnegative sum, product,
composition, and fixed-point majorant preserves domination while the radius
$\rho$ is added, proving validity on the stated ball.  Equation
\eqref{eq:transported-envelope-drift} is the usual cross-entropy
scaled-momentum bound.  Finally
$x_j=b_j+e_j$ with $\|\Pi_\theta e_j\|\le E$, so the logit difference changes
by at most $\sqrt2L_{1,j}E$; $a_j$ belongs to $b_j$ and is known rather than
uncertain.
\end{proof}
...
\begin{corollary}[Chronological parameter-forcing envelope]
\label{cor:causal-parameter-row}
Under the scaled-momentum factorization of
Corollary~\ref{cor:parameter-forcing}, let $\bar K_i$ map transition forcings
through time $i$ to the state error at checkpoint $i+1$, and let
$\mathcal B_i$ apply $\mathcal B$ blockwise to a parameter-forcing prefix.
Define
\begin{equation}
 T_i=\Pi_\theta\bar K_i\mathcal B_i,\qquad
 \kappa_i^\theta\ge\|T_i\|.
 \label{eq:causal-parameter-row}
\end{equation}
Suppose the corrected defect is a known signed state forcing $q$ plus
$\mathcal B\xi$, where $\|\xi_k\|\le\epsilon_k$, and put
\begin{equation}
 y_{i+1}=\Pi_\theta(\bar K_Hq)_{i+1},\qquad
 Y_{i+1}=\|y_{i+1}\|+\kappa_i^\theta
 \left(\sum_{k=0}^{i}\epsilon_k^2\right)^{1/2}.
 \label{eq:causal-row-affine}
\end{equation}
If the objective Hessian at transition $k$ is $L_k$-Lipschitz on its declared
parameter ball, define $r_0=0$ and evaluate
Eq.~\eqref{eq:causal-row-main} chronologically.  Whenever every resulting ball
lies inside its declared domain,
\begin{equation}
 \|\Pi_\theta(x_{a+k}-b_k)\|\le r_k,
 \qquad 1\le k\le H.
 \label{eq:causal-row-tube}
\end{equation}
Computed-response and arithmetic residual bounds enter additively in
$Y_{i+1}$; they do not alter the recursion.
\end{corollary}

\begin{proof}
Write $p_k=\Pi_\theta(x_{a+k}-b_k)$.  Exact scaled-momentum factorization and
causality give
\begin{equation}
 p_{i+1}=y_{i+1}+T_i\bigl(
 \xi_0+R_0(p_0),\ldots,\xi_i+R_i(p_i)\bigr),
 \label{eq:causal-row-identity}
\end{equation}
where $R_k$ is defined in
Eq.~\eqref{eq:objective-gradient-remainder}.  The unresolved known forcing is
bounded by the second term of Eq.~\eqref{eq:causal-row-affine}.  Since $p_0=0$,
the first nonlinear remainder vanishes.  If $\|p_k\|\le r_k$ through time $i$,
the norm of the remaining nonlinear forcing prefix is at most
\begin{equation}
 \left\{\sum_{k=1}^{i}
 \left(\frac{L_kr_k^2}{2}\right)^2\right\}^{1/2}.
\end{equation}
Multiplication by $\|T_i\|$ yields Eq.~\eqref{eq:causal-row-main}; chronological
induction proves Eq.~\eqref{eq:causal-row-tube}.  Adding certified numerical
residuals follows by the same triangle inequality.
\end{proof}
...
before the method seal.  The separate outward verifier begins at zero radius on
...
selection/outcome barrier.  The post-outcome outward verifier directly encloses
the exact-real optimizer map under the arithmetic assumptions in
Appendix~\ref{app:repaired-arithmetic}, without a Green norm or probe.
...
(827 parameters) provide independent outward checks of 16 issued brackets,
and a disjoint mod-13 study (1,933 parameters) supplies HVP-only
active/complement uncertainty.  Direct outward continuations independently
...
The corrected verifier is a separately hashed direct validated-continuation
program combining 192-bit Arb endpoints and outward binary64 error bounds.
Under the arithmetic assumptions in Appendix~\ref{app:repaired-arithmetic},
it treats every stored
...
largest optimizer-Jacobian norm bound is 1.00744, the largest basis
orthogonality bound is $7.53\times10^{-12}$, and the largest
Hessian--basis residual bound is $1.68\times10^{-11}$.  All 56 singleton brackets remain identical and all
56 cover.
...
\begin{table}[H]
\caption{Historical WDBC timing on the 56 Green-issued coordinates.
The direct row predates the arithmetic repair; repaired replay costs are
reported separately in Appendix~\ref{app:repaired-arithmetic}.  Direct timing
excludes construction of the shared four-sweep centerline.}
...
Historical direct implementation & 56 & 56/56 & none & 7.12 h \\
\bottomrule
\end{tabular}
\end{table}

The historical direct/GreenCert aggregate ratio is $12.61\times$; it is not a
timing ratio for the corrected verifier.  The direct comparator was not run on
the 15 Green abstentions.  Both methods use the same four-sweep reference;
only the direct one-step tube dispenses with the Green closure.  Exact-real
scope begins at the stored checkpoint, under the stated arithmetic assumptions.
...
A post-seal verifier propagates a zero-radius outward tube around each sealed
three-sweep path under the exact-real full-batch map and stated arithmetic
assumptions.  Portable replay uses byte-authenticated reference and dataset
arrays rather than assuming cross-platform bitwise reconstruction.  It imports
...
maximum Hessian--basis residual bound is $1.22\times10^{-10}$, and maximum
basis orthogonality bound is $2.89\times10^{-11}$.  This includes the
...
The chronological row protocol was then frozen at commit
\texttt{5aff03c2515e9256} after development on seed 373's 70\% case and before
any structured-row query on the other 14 panel rows.  It fixes four variational
sweeps, the signed directional quadratic and recurrence forcing, mixed
fourth-order residuals, one eight-probe stream per candidate, and a 4/8
forward-only stopping rule.  Every row issues its sealed bracket: 13 at four
probes and two at eight.  The second eight-probe row is the held-out seed-373
80\% case, which retains $[214,214]$.  The aggregate SHA-256 begins
\texttt{72956D00921CEF63}; a separate verifier bound to the frozen source and
all 17 raw blocks recomputes 15/15 decisions and the fair 128/68, 129/83, and
144/98 cost decompositions.  Its maximum radius/domain ratio is
$1.284\times10^{-6}$ and it reads no future outcome.

Separately, Proposition~\ref{prop:causal-prefix} is executable rather than only
...
\begin{table}[H]
\caption{Outcome-blind audit of Proposition~\ref{prop:transported-neural-envelope}.
The dominance ratios compare the transported majorant with direct float64
evaluation at the corrected center; theorem validity follows from monotonicity,
not from these numerical comparisons.}
\label{tab:transported-envelope-audit}
\centering
\small
\setlength{\tabcolsep}{8pt}
\begin{tabular}{lr}
...
Quantity & Result \\
\midrule
Cases / nondevelopment cases & 4 / 3 \\
Stage, geometry, and derivative checks & 9,420 \\
Issued / sealed brackets retained & 4/4 / 4/4 \\
Issued offsets & 2, 28, 70, 118 \\
Maximum stage-majorant ratio & $1+7.32\times10^{-13}$ \\
Maximum derivative-envelope ratio & $1+5.53\times10^{-14}$ \\
Outcome files / new randomized queries & 0 / 0 \\
\bottomrule
\end{tabular}
\end{table}
...
before that trajectory is opened.  The small-model outward continuations
provide an independent check of the event brackets; their arithmetic and
execution costs are reported separately in Appendix~\ref{app:repaired-arithmetic}.
...
survive a separate outward pass unchanged under the arithmetic assumptions
in Appendix~\ref{app:repaired-arithmetic}.
...
$\kappa$, probe, or random variable.  The corrected replay and independent
scalar/output checks in Appendix~\ref{app:repaired-arithmetic} retain all
40 zero-radius anchor tubes.
...
audit verifies all seven corrected direct tubes, cache hashes, positive logic
...
The causal-row audit is bound to frozen commit
\texttt{5aff03c2515e9256}; its runner refuses a dirty tracked tree, runs five
preaudit tests, and hashes every claim-bearing source.  From 17 raw blocks, an
independent scalar verifier reconstructs all 15 calibrations, chronological
radii, persistent brackets, cache identities, and costs with zero outcome reads.
...
\subsection{Corrected arithmetic and independent continuation replay}
\label{app:repaired-arithmetic}

The continuation verifier combines 192-bit Arb transcendental endpoints with
outward binary64 elementary, sum, and matrix-product bounds.  A numerical
dependency audit found that the earlier matrix-product bound used a relative
rounding term alone and omitted accumulated subnormal error.  We retained the
counterexample and original records, replaced the primitive in a separately
identified verifier, and replayed all 79 affected historical claims: 56 WDBC,
seven digits, and 16 dense modular-MLP appendix events.  Every original bracket
is retained.  This repair changes neither prospective issuance nor the 83-event
headline population; the 16 appendix events remain a separate study.

For an ordinary length-$n$ binary64 dot product, the corrected bound includes
both $\gamma_D\sum_i|a_i b_i|$ and an additive underflow allowance
$D\eta/(1-Du)$, with $D=2n+8$, $u=2^{-53}$, and $\eta=2^{-1074}$.
It assumes round-to-nearest, gradual underflow, no internal overflow, and at
most $D$ rounded operations; nonfinite enclosures cause refusal.  Runtime
checks test necessary arithmetic conditions, not the implementation of an
arbitrary BLAS binary.  These assumptions delimit the exact-real continuation
claim even though the Arb portions use arbitrary precision.

A second audit replays all 7,551 state-radius and optimizer-norm inequalities
with exact rational arithmetic.  An independent scalar-Arb implementation,
using no BLAS products, rechecks 1,485,052 output margins at 384 bits and
reconstructs each persistent bracket from integer count bounds.  The output
audit is conditional on the authenticated state tubes; it does not independently
rederive their neural derivative bounds.  The fixed-ownership ledger excludes
duplicate portability pilots and retains execution identities across machines.
Its SHA-256 prefix is \texttt{6E0EE195EAE512980}; the descriptive summary is
\texttt{697F36AD8503486D}.  Both full hashes and replay commands are supplied
with the code.

The repaired runs cover 40 WDBC tubes/5,089 transitions, seven digits
tubes/610 transitions, and 16 modular tubes/1,852 transitions.  Their maximum
state radii are $4.47\times10^{-13}$, $5.07\times10^{-9}$, and
$6.68\times10^{-5}$, respectively.  Runs used Windows/x86 and Linux/ARM;
the released per-job timings retain this distinction and are not pooled into
a controlled speedup comparison.  Byte-authenticated reference paths and
dataset tensors make replay independent of cross-platform bitwise training
reconstruction.

\paragraph{Exact-real boundary.}
Under the arithmetic assumptions above, the WDBC, digits, and dense-modular
passes certify continuation from exact stored dyadic checkpoints, not the
PyTorch/BLAS training process that produced them.  The
...
Modular data are exhaustive; WDBC and digits carry checksums and persistent UCI
identifiers.  The supplement provides source, tests, seals, manifests, audits,
and exact commands; large checkpoint arrays are regenerated.
