...
$M$-Lipschitz, one sweep contracts the maximum path defect quadratically:
\begin{equation}
 \max_j\norm{r^{(k+1)}_j}
...
classified at update $a+j$.  A gate requires $n_j\ge r$.  Its $K$-persistent
first passage is
\begin{equation}
 T_{r,K}=\min\{j:n_{j+\ell}\ge r\text{ for every }0\le\ell<K\}.
 \label{eq:event}
\end{equation}
The scientific object is a bracket $[L,U]$ guaranteed to contain $T_{r,K}$,
or abstention.
...
\begin{align}
 L&=\min\{j:n^+_{j+\ell}\ge r\ \forall 0\le\ell<K\},\nonumber\\
 U&=\min\{j:n^-_{j+\ell}\ge r\ \forall 0\le\ell<K\}.
 \label{eq:persistentbracket}
\end{align}
Whenever both exist, $L\le T_{r,K}\le U$.  No monotonicity of accuracy is
assumed.
...
scale \citep{kodali2026residual}.  Proposition~\ref{prop:inexact-probe}
...
If $n^-_j\le n_j\le n^+_j$ for every update and $L,U$ are defined by
Eq.~\eqref{eq:persistentbracket}, then $L\le T_{r,K}\le U$.
\end{proposition}
