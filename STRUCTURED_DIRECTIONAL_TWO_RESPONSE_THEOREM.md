...
Jacobians (ar J_0,ldots,ar J_{H-1}:E\to E), the anchor-fixed causal
Green operator (ar K_H:E^H\to E^H) maps a forcing sequence
(f=(f_0,ldots,f_{H-1})) to (y=(y_1,ldots,y_H)), where

\[
y_0=0,qquad y_{j+1}=ar J_jy_j+f_j.
\]
...
r_j={\rho_j^{(w)}-\rho_j^{(\theta)}\over2\eta},qquad
c_j={\rho_j^{(\theta)}+\rho_j^{(w)}\over2}.
\]
...
\[
\widetilde q_j={1\over2}D^3F(\theta_j)[a_j,a_j,\cdot].
\]
...
\sigma_q\le
\left(\sum_{j=0}^{H-1}
\left({L_{F,4,j}\over6}\|a_j\|^3\right)^2\right)^{1/2}.
}
\]
