#!/usr/bin/env python3
"""Frozen constants and operator identities for Transformer v3 confirmation."""
...
def maximum_operator_count() -> dict:
    ...
    return {
        ...
        "power_levels_per_operator": MAXIMUM_POWER,
        "power_levels_share_one_simultaneous_event": True,
    }
...
def probe_config() -> ProbeConfig:
    ...
    return ProbeConfig(
        PROBES,
        MAXIMUM_POWER,
        FAMILY_FAILURE_PROBABILITY / count,
    )


def output_identity(candidate: Candidate, step: int) -> tuple[int, ...]:
    if candidate.seed not in SEEDS:
        raise ValueError("candidate seed is outside the v3 population")
...
def candidate_universe(
    ...
    for candidate in candidates:
        ...
        if pair in seen:
            raise ValueError(f"multiple v3 candidates for one seed/gate: {pair}")
        seen.add(pair)
