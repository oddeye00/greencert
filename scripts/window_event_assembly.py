"""Strict, model-independent assembly of finite-window event bounds.
...
from response_centered_scalar_closure import solve
...
class MarginBound:
    ...
    lower: float
    upper: float
    point_jacobian_upper: float
    ball_hessian_upper: float
...
def _number(value, name, nonnegative=True):
    _require(type(value) in (float, int), f"missing_or_invalid_{name}")
    try:
        result = float(value)
    except OverflowError:
        raise AssemblyError(f"nonfinite_{name}") from None
    _require(math.isfinite(result), f"nonfinite_{name}")
...
def assemble(*, identity, training_count, example_ids, domain, drift_rows,
    ...
    """
    base = {"conditional_on_supplied_bounds": True, "artifact_authentication_performed": False,
    ...
    base |= {"inputs_compatible": True, "state_closure": closure,
             "probability_scope": green.probability_scope, "failure_probability": green.failure_probability}
    ...
    try:
        ...
        for j in range(identity.horizon + 1):
            ...
            r = arb(0) if j == 0 else arb(p[j]) + arb(closure["radius"])
            lo = hi = 0
            for margin in outputs[j].margins:
                if j == 0:
                    ...
                    uncertainty = arb(2).sqrt() * (arb(margin.point_jacobian_upper) * r +
                        arb(margin.ball_hessian_upper) * r * r / 2)
                lower = arb(margin.lower) - uncertainty
                upper = arb(margin.upper) + uncertainty
                lo += int(bool(lower > 0))
